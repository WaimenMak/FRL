...
class TD3(object):
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        state = torch.tensor(
            state, device=device, dtype=torch.float)  # bc * state_dim
        action = torch.tensor(
            action, device=device, dtype=torch.float)  # bc * action_dim
        reward = torch.tensor(
            reward, device=device, dtype=torch.float).view(-1, 1)
        next_state = torch.tensor(
            next_state, device=device, dtype=torch.float)
        not_done = torch.tensor(np.float32(not_done), device=device, dtype=torch.float).view(-1, 1)
...
if __name__ == '__main__':
    ...
    agent = TD3(state_dim, action_dim, args.action_bound)
    replay_buffer = replay_buffer(args.capacity)
    for i_ep in range(args.episode_num):
        ...
        for iter in range(args.episode_length):
            ...
            action = (
                ...
                + np.random.normal(0, args.action_bound * args.std_noise, size=action_dim)
            ).clip(-args.action_bound, args.action_bound)

            n_state, reward, done, _ = env.step(action)  # env.step accept array or list
            replay_buffer.add(state, action, n_state, reward, done)
            agent.train(replay_buffer, args.local_bc)
            # agent.memory.add(state, action, reward, n_state, done)
            # state_bc = agent.UpdateQ()
            ep_reward += reward
            if done == True:
                break
            state = n_state
