# -*- coding: utf-8 -*-
# @Time    : 2021/12/22 19:04
# @Author  : Weiming Mai
# @FileName: fed2_origin.py
# @Software: PyCharm
#origin implementation of fed2_td3,图片是jupyter 前三副图的代码
import os
import sys
import torch
...
class Arguments():
    def __init__(self):
        ...
        # self.capacity = 1e6
        self.capacity = 10000
        self.N = int(1)   #send q frequency (playing_step // N), playing_step:update q frequency
        self.M = 5   #update mu frequency (playing_step // M)
        self.L = int(1)   #send mu frequency (playing_step // M // L)
        self.Round = self.playing_step // self.N + self.playing_step // self.M // self.L
        # self.C_iter = 5
        self.client_num = 5
        self.filename = f"evalori_{self.env_seed}_N{self.N}_M{self.M}_L{self.L}"  #filename:env_seed, model_name:env_name
