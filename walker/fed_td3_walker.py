...
class Arguments():
    def __init__(self):
        ...
        # self.C_iter = 5
        self.client_num = 5
        self.filename = f"evalfed1_{self.env_seed}_N{self.N}_M{self.M}_L{self.L}"  #filename:env_seed, model_name:env_name
...
def ClientUpdate(client_pipe, agent, local_env, args):
    ...
    for i_ep in range(args.playing_step):
        ...
        if (i_ep+1) % args.N == 0:  #update Q
            ...
            # agent.to_cpu([q])
            # client_pipe.send((q.state_dict(), False))  # send Q, target: false
            # global_q = client_pipe.recv()                     # recv agg Q
            # q.load_state_dict(global_q)
            # agent.to_gpu([q])
...
if __name__ == '__main__':
    args = Arguments()
    print(f"client:{args.client_num},N:{args.N},M:{args.M},L:{args.L}")
    env = PendulumEnv()
    # env = BipedalWalker()
    set_seed(1)
    state_dim = env.observation_space.shape[0]
