# -*- coding: utf-8 -*-
# @Time    : 2021/12/22 13:43
# @Author  : Weiming Mai
# @FileName: fedTD3_cpu.py
# @Software: PyCharm
    ...
    def choose_action2(self, state):
        ...
        with torch.no_grad():
            ...
            noise = torch.tensor(np.random.normal(0, self.std_policy_noise, size=[state.size(0), self.action_dim]).clip(
                    -self.noise_clip,self.noise_clip), dtype=torch.float)
            action = (
...
class fedTD3():
    ...
    def UpdateQ(self, args, client_pipe, step):
        ...
        if self.iter % args.M == 0:
            # self.localDelayUpdate(state_batch, self.critic.Q_net, self.tau, client_pipe, step)
