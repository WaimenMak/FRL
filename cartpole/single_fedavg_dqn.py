...
ex = Experiment("fed_dqn_identical_env")  #name of the experiment
...
def Config():
    ...
    args.env_name = 'nonstaion_Cartpole'
    ...
    args.filename = "_".join(('seed',str(args.env_seed),'feddqn.pth'))
    seed = 1
    #FedAvg Parameters
...
def agent_env_config(args, seed, env_name):
    if env_name == 'CartPole-v1':
        ...
        env = cart_env_config(seed) # seed
    # env.seed(args.train_seed)
    state_dim = env.observation_space.shape[0]  # 4
    action_dim = env.action_space.n  # 2
    agent = fed_DQN(state_dim, action_dim, args)
    return agent, env
...
def GenerateAgent(args):
    ...
    for i in range(args.client_num):
        agent, local_env = agent_env_config(args, seed=args.env_seed, env_name=args.env_name)
...
def main(args, seed):
    ...
    glob_env = cart_env_config(args.env_seed)
    set_seed(seed)
    scheduler = None
    if args.scheduler:
