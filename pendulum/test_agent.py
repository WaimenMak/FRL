...
ex = Experiment("test_pendulum")  #name of the experiment
...
def Config():
    ...
    args = Arguments()
    args.epsilon = 0
    args.local_bc = 0 #local update memory batch size
    args.local_epi = 0 # local update episode 最好是C_iter倍数
    args.gamma = 0
    args.lr = 0
    # args.episode_num = 50
...
def eval(length, agent, env, eval_episode, name, log=True):
    ...
    for i_ep in range(eval_episode):
        ...
        for _ in range(length):
            # env.render()
            action = action_trans(agent.predict(state), agent.action_dim, upperbound, lowerbound)
...
def eval_agents(args, local_envs, agents, eval_episode, fresh_agent):
    ...
    for env in total_envs:
        ...
        k += 1
    return total_mean_reward/(k)
@ex.automain
def main_eval(args):
    model_path = '../outputs/fed_model/'
    # model_path = '../outputs/model/pendulum/'
    action_dim = 11
    glob_env = pendulum_env_config(0)
    fresh_agent = fed_DQN(glob_env.observation_space.shape[0], action_dim, args)

    fresh_agent.name = 'agent_server'
    # file_name = 'seed_0_pend_feddqn.pth'
    file_name = 'clients_5_Pendulum_fedavgdqn.pth'
    # file_name = 'seed_0_dqn_pendulum.pth'
    ...
    agents = []
    for i in range(10):
        local_envs.append(pendulum_env_config(i))
    # local_envs.append(pendulum_env_config(0))
