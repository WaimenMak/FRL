...
from sccd_conv import fedTD3, Actor
...
class Arguments():
    def __init__(self):
        ...
        self.actor_dual = False
        self.actor_partial = 0.9
        self.actor_epc = 20
        ######################
        ...
        if self.env_name == "carrace":
            ...
            self.episode_length = 1000  # env._max_episode_steps
            self.playing_step = int(1e6)
            self.capacity = 1e5  #10000
            self.std = 2        #env_param
        ...
        # self.env_seed = None
        self.filename = f"distilstd{self.std}_noicy{self.noisy_input}_{self.playing_step}_{self.env_name}{self.env_seed}_N{self.N}_M{self.M}_L{self.L}_criticdual{self.critic_dual}{self.critic_partial}_epc{self.critic_epc}_lr{self.lr}_actordual{self.actor_dual}{self.actor_partial}_reweight{self.reweight}_distepoch{self.epochs}_lrdecay{self.schedul}"  #filename:env_seed, model_name:env_name
...
def ClientUpdate(client_pipe, agent, local_env, args):
    ...
    for i_ep in range(args.playing_step):
        ...
        if (i_ep+1) % args.N == 0:  #update Q
            with torch.no_grad():
                #action can be encrpted or just original, these are image representation
                dist_rep1, dist_rep2 = agent.critic.Q_net.client_rep(state_batch)
                # dist_rep1 = torch.cat([dist_rep1, action_batch], dim=1)
                # dist_rep2 = torch.cat([dist_rep2, action_batch], dim=1)
                ...
                #### for the output of the activation
                # dist_rep1[dist_rep1 < 0] = 0
                # dist_rep2[dist_rep2 < 0] = 0

                #### concat action as the new representation (all tensor)
                dist_rep1 = torch.cat([dist_rep1, action_batch.cpu()], dim=1)
                dist_rep2 = torch.cat([dist_rep2, action_batch.cpu()], dim=1)
            ...
            prev_q, global_q, frac = client_pipe.recv()  # recv agg Q
...
# def critic_distill(local, glob, state, action, partial)
