# -*- coding: utf-8 -*-
# @Time    : 2022/2/26 15:34
# @Author  : Weiming Mai
# @FileName: reweight_fedavg.py
# @Software: PyCharm
...
class Arguments():
    def __init__(self):
        ...
        # self.env_seed = None
        self.filename = f"reweight{self.reweight_tau}niidfedstd{self.std}_noicy{self.noisy_input}_{self.playing_step}_{self.env_name}{self.env_seed}_N{self.N}_M{self.M}_L{self.L}_beta{self.beta}_mu{self.mu}_dual:{self.dual}_lrdecay{self.schedul}"  #filename:env_seed, model_name:env_name
...
def GenerateAgent(args):
    ...
    for i in range(args.client_num):
        if not args.niid:
            ...
            agent, local_env = agent_env_config(args, seed=i+7)
...
def ClientUpdate(client_pipe, agent, local_env, args):
    ...
    lc_env = deepcopy(local_env)    #for evaluation
    ...
    for i_ep in range(args.playing_step):
        ...
        if (i_ep+1) % args.episode_length // 2 == 0:       # this evaluation maybe necessary if we use local distillation
            # n += 1
            reward_log = eval2(agent, [lc_env], args)
            eval_reward.append(reward_log)
            np.save(f"{model_path}env_std{args.std}agent{agent.name}", eval_reward)
...
def ServerUpdate(pipe_dict, server, weighted, actor, envs, args): #FedAvg
    ...
    for round_ in range(args.Round):
        ...
        if not target:
            Agg(local_models, server.q.state_dict(), re_weight, args)
            for i in range(args.client_num):
