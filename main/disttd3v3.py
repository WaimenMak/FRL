...
# @Time    : 2022/3/4 15:59
# @Author  : Weiming Mai
# @FileName: disttd3v3.py
# @Software: PyCharm

# -*- coding: utf-8 -*-
# @Time    : 2022/1/12 18:15
# @Author  : Weiming Mai
# @FileName: dist_td3.py
# @Software: PyCharm
...
class fedTD3():
    def __init__(self, state_dim, action_dim, args):
        ...
        self.q_update_total_times = args.playing_step
        self.count = 0   #policy network local update times, count*M/N --> current q rounds
        self.L = args.L
    ...
    def actor_distill(self, Q_net, state, partial):
        loss1 = -Q_net.Q1_val(state, self.actor.policy_net(state)).mean()
        loss2 = torch.sum(
            (self.actor.glob_mu(state) - self.actor.policy_net(state)) ** 2).mean()
    ...
    def localDelayUpdate(self, state, Q_net, tau, client_pipe):
        ...
        if self.count % self.L == 0: # Time to send policy netwok
            ...
            ####### local distill #######
            if self.actor_dual and (self.count * 2 / self.q_update_total_times) > 0.75:
                self.actor.policy_net.load_state_dict(mu_params)   #first load agg policy, and then further maximize q value
                #=======================================
            ...
            #2.##########################
            ###### further update policy network to consist to the Q model
            # if (self.count * 2 / self.q_update_total_times) > 0.75:
                for epc in range(self.actor_epc):  # actor_epc = 1,2,3 not too much
                    state_batch, _, _, _, _ = self.memory.sample(
                        self.batch_size)
                    state_batch = torch.tensor(
                        np.array(state_batch), device=self.device, dtype=torch.float)  # bc * state_dim
                    self.actor_distill(Q_net, state_batch, self.actor_partial)
                    # state_batch, _, _, _, _ = self.memory.sample(
                    #     self.batch_size)
                    # state_batch = torch.tensor(
                    #     np.array(state_batch), device=self.device, dtype=torch.float)  # bc * state_dim
                    # self.actor.update_policy(state_batch, Q_net)
