...
class fedTD3():
    ...
    #     loss = self.critics_loss(current_q_val[0], y_hat) + self.critics_loss(current_q_val[1],y_hat) + self.alpha * (self.l_mse(current_q_val[0], g_q1) + self.l_mse(current_q_val[1], g_q2))
        ...
        # elif self.choice == 'KL':
        #     states = torch.tensor(np.tile(states, (1, self.n)).reshape(self.n * states.shape[0], states.shape[1]),
        #                           device=self.device, dtype=torch.float)
        #     noise = torch.tensor(np.random.normal(0, 0.5, size=[state_batch.size(0), self.n * self.action_dim]).clip(
        #             -self.noise_clip,self.noise_clip), dtype=torch.float).cuda()
        #     actions = (
        #         action_batch + noise            # noise is tensor on gpu
        #     ).clip(-self.action_bound, self.action_bound)
        #
        #     loss = self.critics_loss(current_q_val[0], y_hat) + self.critics_loss(current_q_val[1], y_hat) + self.alpha * KL(self.critic.Q_net, self.glob_q, states, actions, self.softmax, self.n)
