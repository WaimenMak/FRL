# -*- coding: utf-8 -*-
# @Time    : 2022/1/12 16:33
# @Author  : Weiming Mai
# @FileName: dist_reg.py
# @Software: PyCharm
...
def ClientUpdate(client_pipe, agent, local_env, args):
    ...
    for i_ep in range(args.playing_step):
        ...
        if (i_ep+1) % args.N == 0:  #update Q
            ...
            dist_rep1 = dist_rep1.cpu().detach()
            dist_rep2 = dist_rep2.cpu().detach()
            q_label1 = q_label1.cpu().detach()
            q_label2 = q_label2.cpu().detach()
            ######## distillation ########
...
def ServerUpdate(pipe_dict, server, weighted, actor, envs, args): #FedAvg
    ...
    for round_ in range(args.Round):
        ...
        if not target:
            ...
            for i in range(args.client_num):
                pipe_dict[i][1].send((server.mu.state_dict(), (server.q.oupt_layer_q1.state_dict(), server.q.oupt_layer_q2.state_dict())))
