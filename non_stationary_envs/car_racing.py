...
from non_stationary_envs.car_dynamics import Car
...
class CarRacing(gym.Env, EzPickle):
    ...
    def _set_config(self,
        ...
        if self.discretize_actions == "soft":
            ...
            self.action_space = spaces.Box(np.array([-1, min_speed, 0]), np.array([+1, +1, +1]),
    ...
    def step(self, action):
        ...
        # self._update_state(self.get_arr("state_pixels"))
    ...
    def render(self, mode='human'):
        if self.viewer is None:
            ...
            self.full_score_label = pyglet.text.Label('Full Score: 0000', font_size=20,
                                                      x=250, y=WINDOW_H * 1.5 / 40.00, anchor_x='left',
                                                      anchor_y='center',
                                                      color=(255, 255, 255, 255))
...
class Arguments():
    def __init__(self):
        ...
        self.filename = "v3_distilstd0_noicyFalse_1200000_walker5_N400_M2_L400_criticdualTrue0.1_actordualFalse0.9_reweightFalse1_distepoch20_lrdecayFalse_actor_1actor_"
...
def get_state(obs):
    state = torch.tensor(np.array(obs), dtype=torch.float).to(args.device)
    # state = state.transpose((2, 0, 1))
    state = state.permute(2, 0, 1)  # 4 * 96 *96

    return state.unsqueeze(0)

if __name__ == "__main__":
    from models.Conv_model import conv_policy
    import torch
    ...
    # action_bound = torch.tensor([[-1., 0., 0.], [1., 1., 1.]]).to(args.device)
    # agent = conv_policy(4, 3, action_bound)
    ...
    #         action = torch.squeeze(agent(state).detach()).cpu().numpy()
