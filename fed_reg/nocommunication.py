...
class Arguments():
    def __init__(self):
        ...
        # self.env_seed = None
        self.filename = f"niidevalfedstd{self.std}_noicy{self.noisy_input}_{self.playing_step}_{self.env_name}{self.env_seed}_N{self.N}_M{self.M}_L{self.L}_beta{self.beta}_mu{self.mu}_dist:{self.dual}"  #filename:env_seed, model_name:env_name
...
def ClientUpdate(client_pipe, agent, local_env, args, envall):
    """
    A process function
    :param client_pipe: 
    :param args: 
    ...
    for i_ep in range(args.playing_step):
        ...
        if (i_ep+1) % args.episode_length == 0:       # this evaluation maybe necessary if we use local distillation
            if agent.name=="agent1":
                n += 1
                reward_log = eval(agent, envall, args)
                print(f"train_episode{n}_{agent.name}:{reward_log:.2f}")
...
if __name__ == '__main__':
    ...
    client_process_list = []
    for i in range(args.client_num):
        pro = Process(target=ClientUpdate, args=(pipe_dict[i][0], agents[i], local_envs[i], args, local_envs))
