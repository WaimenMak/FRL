...
def Config():
    ...
    args.local_epi = 50 # local update episode 最好是C_iter倍数
    args.gamma = 0.98
    args.lr = 0.002
    args.episode_num = 50
...
def ClientUpdate(args, agent, k, params, local_env):
    ...
    for i_ep in range(args.local_epi):
        ...
        r += eval(agent, local_env, eval_episode=1, name=agent.name +'_eval_ep_reward')

    return r/args.local_epi

def ClientUpdate_2(args, agent, k, params, local_env, n):
    '''
    :param agent: dqn class
    :param params: global net state dict
    :param args: hyper parameters
...
def main(args, seed):
    ...
    for round in range(args.round):
        ...
        for idx in idxs_clients:
            ...
            ClientUpdate_2(args, agents[idx], idx, net_glob, local_envs[idx], round)
        # weighted = list(map(lambda x: x/sum(weighted), weighted))
    ...
    fresh_agent.load(model_path + 'fed_dqn.pth')
    #Testing
